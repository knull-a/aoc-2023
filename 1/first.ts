...
const input1: string = `dssmtmrkonedbbhdhjbf9hq
...
function trebushet1(arrInput: string[]) {
  let sum = 0;
  for (let i = 0; i < arrInput.length; i++) {
    ...
    const arrNumbers = arrInput[i]
      .split("")
      .filter((item) => !isNaN(Number(item)));
    const newSum = arrNumbers[0] + arrNumbers.slice(-1)[0];
    sum += Number(newSum);
    console.log(arrNumbers[0], arrNumbers.slice(-1)[0])
  }
  console.log(sum);
}

trebushet1(input1.replace(/\n/g, ' ').split(" "));
